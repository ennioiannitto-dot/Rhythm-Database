...
const STORAGE_KEY = 'tongueApp:data';
...
el('#btn-export').onclick = ()=>{
  ...
  a.href = url; a.download = `tongue-backup-${stamp}.json`;
...
/* Spaziatura dinamica in fondo alla pagina, per non far coprire mai il
   contenuto dalla barra di navigazione fissa (la sua altezza reale varia
   in base a browser, zoom e safe-area del dispositivo). */
function adjustBottomSpacing(){
  const bar = document.querySelector('.bottombar');
  if(!bar) return;
  document.documentElement.style.setProperty('--bottombar-h', bar.offsetHeight+'px');
}
...
if('serviceWorker' in navigator){
  window.addEventListener('load', ()=>{
    navigator.serviceWorker.register('sw.js').catch(()=>{ /* funziona comunque online */ });
  });
}
